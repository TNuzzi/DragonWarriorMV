...
Scene_Boot.prototype.loadSystemWindowImage = function() {
    ...
    ImageManager.requestPicture('dark-default');
    ImageManager.requestPicture('dark-torch');
    ImageManager.requestPicture('dark-radiant-1');
    ImageManager.requestPicture('dark-radiant-2');
    ImageManager.loadBattleback2('dw1_battle_opening');
    ImageManager.loadBattleback2('dw1_battle_opening_dark');
    ImageManager.loadBattleback2('dw1_battle_opening_dark_cave');
    ImageManager.loadBattleback2('dw1_battleback');
    ImageManager.loadBattleback2('dw1_battleback_dark');
    ImageManager.loadBattleback2('dw1_battleback_dark_cave');
};
...
Scene_Title.prototype.start = function() {
    ...
    if(StorageManager.exists(1)) {
        ...
        $gamePlayer.reserveTransfer(41, 0, 1, 8, 2);
        $gameSystem.onAfterLoad();
        $gameSwitches.setValue(45, true); // Return from continue game
    ...
    $gameSwitches.setValue(84, true); // On title screen
    $gameSwitches.setValue(85, false); // Words on title screen
    $gameSwitches.setValue(86, false); // Title select screen
...
Scene_Map.prototype.initialize = function() {
    ...
    this._battleBackWidth = 336;
    this._battleBackHeight = 243;
};
...
Scene_Map.prototype.terminate = function() {
    ...
    if (!SceneManager.isNextScene(Scene_Battle)) {
        ...
        if (!$gameSwitches.value(10)) {
            this._battleBack.setFrame(0, 0, this._battleBackWidth, this._battleBackHeight);
        }
    }
...
Scene_Map.prototype.startEncounterEffect = function() {
    ...
    this._encounterEffectDuration = 50;
    this._animationCount = 0;
    this.snapForBattleBackground(); // Comment out to make black
    var imageWidth = this._battleBackWidth;
    var imageHeight = this._battleBackHeight;
    
    if ($gameSwitches.value(10)) {
        imageHeight = 339;
    }

    this._battleBack = new Sprite();

    // if in a cave use a dark battleback 
    if ($gameSwitches.value(10)) {
        this._battleBack.bitmap = ImageManager.loadBattleback2('dw1_battle_opening_dark_cave');
    } else if ($gameSwitches.value(65)) {
        this._battleBack.bitmap = ImageManager.loadBattleback2('dw1_battle_opening_dark');
    } else {
        this._battleBack.bitmap = ImageManager.loadBattleback2('dw1_battle_opening');
    }
    
    this._battleBack.x = Math.floor((Graphics.width - imageWidth) / 2);
    this._battleBack.y = Math.floor((Graphics.height - imageHeight) / 2);
    this._battleBack.setFrame(0, 0, imageWidth, imageHeight);
    this.addChild(this._battleBack);
...
Scene_Map.prototype.updateEncounterEffect = function() {
    ...
    var pw = imageWidth;
    var ph = imageHeight;
    var sx = this._animationCount % 5 * pw;
    var sy = Math.floor(this._animationCount / 5) * ph;

    this._animationCount++;
    this._battleBack.setFrame(sx, sy, pw, ph);
};
...
Scene_Battle.prototype.updateBattleProcess = function() {
    ...
    this.changeInputWindow();  // Comment out
};
...
Scene_Battle.prototype.isAnyInputWindowActive = function() {
    return (this._partyCommandWindow.active ||
            ...
            this._dwCommandWindow.active);
    // return false;
};

Scene_Battle.prototype.changeInputWindow = function() {
    if (BattleManager.isInputting()) {
        if (!this._dwCommandWindow.active && 
            !this._dwSpellWindow.active &&
            !this._dwItemWindow.active) {
            this._logWindow.displayCommandWindow();
        }
    } else {
...
Scene_Battle.prototype.createDisplayObjects = function() {
    ...
    BattleManager.setDWStatusWindow(this._dwStatusWindow);
    BattleManager.setDWCommandWindow(this._dwCommandWindow);
