...
Game_Screen.prototype.startFlash = function(color, duration, times) {
    this._flashColor = color.clone();
    this._flashDuration = duration;
    this._flashOriginalColor = color.clone();
    this._flashOriginalDuration = duration;
...
Game_Screen.prototype.updateFlash = function() {
    ...
    if (this._flashDuration <= 0 && this._flashTimes > 0) {
        this._flashTimes--;
        this._flashColor = this._flashOriginalColor.clone();
        this._flashDuration = this._flashOriginalDuration;
    }
};

Game_Screen.prototype.updateShake = function() {
    if (this._shakeDuration > 0) {
        if (this._shakeDuration % 8 === 0) {
            SceneManager._scene.y = 5;
        } else if (this._shakeDuration % 8 === 7) {
            SceneManager._scene.y = 0;
        } else if (this._shakeDuration % 8 === 6) {
            SceneManager._scene.x = -5;
        } else if (this._shakeDuration % 8 === 5) {
            SceneManager._scene.x = 0;
        } else if (this._shakeDuration % 8 === 4) {
            SceneManager._scene.y = -5;
        } else if (this._shakeDuration % 8 === 3) {
            SceneManager._scene.y = 0;
        } else if (this._shakeDuration % 8 === 2) {
            SceneManager._scene.x = 5;
        } else if (this._shakeDuration % 8 === 1) {
            SceneManager._scene.x = 0;
        }
...
Game_Screen.prototype.startFlashForDeath = function() {
    this.startFlash([255, 0, 0, 225], 8);
};
...
Game_Action.prototype.apply = function(target) {
    ...
    result.clear();
    result.failedSpell = false;
    result.used = this.testApply(target);

    /**
     * Golem is not allowed to dodge attacks.
     */
    if (!target.isActor() && target.enemyId() !== 25) {
        result.missed = $gameSystem.randomNum(1, 64) <= target.dodge;
    } else {
        result.missed = false;
    }

    result.evaded = false; // Not used (using missed for enemies dodging the attack)
    result.physical = this.isPhysical();
    result.drain = this.isDrain();
    if (result.isHit()) {
        if (this.isPhysical()) {

            // Excellent Move! does not work Dragonlord
            if (this.subject().isActor() && (target.enemyId() !== 39 && target.enemyId() !== 40)) {
                result.critical = this.subject().isActor() ? $gameSystem.randomNum(1, 32) === 1 : false; // 1 in 32 chances of critical hit
            }
            
            var value = this.makeDamageValue(target, result.critical);
            this.executeDamage(target, value);
        
        } else if(this.subject().hasSpellsBlocked() && 
                  DataManager.isSkill(this.item())) {
            result.missed = false;
            result.drain = false;
            result.critical = false;
            result.spellBlocked = true;
        } else if (this.item().damage.type > 0) {
            result.missed = false;
            result.drain = false;
            result.critical = false;
            ...
            if (!target.isActor() &&
                DataManager.isSkill(this.item()) &&
               (this.item().id === 3 || this.item().id === 11)) { // Determine Hurt resistance
                var hurtResist = target.hurtResist;
                var hurtChance = $gameSystem.randomNum(0, 15);

                if (hurtResist >= hurtChance ) {
                    result.failedSpell = true;
                    return; // end action
                }
            }

            var value = Math.max(eval(this.item().damage.formula), 0)
            if ((this.subject().isActor() && target.isActor()) ||
                (!this.subject().isActor() && !target.isActor())) {
                value = -value;
            }
            
            /**
             * Erdrick's armor will reduce damage by 2/3 
             * for hurt/hurtmore and fire breath
             */
            if (target.isActor() && 
                target.hasErdrickArmor() &&
                (this.item().id === 3 || 
                 this.item().id === 11 || 
                 this.item().id === 16 ||
                 this.item().id === 17)) {
                    value = Math.floor(value * 0.34);
            }
            ...
            switch (this.item().id) {
                case 4: // sleep spell
                    if(target.isAsleep()) {
                        this.makeSuccess(target); // If asleep, sleep spell will always work
                    } else if (target.isActor()) { // Hero will always be put asleep
                        target.addState(5); // Put Hero asleep
                        this.makeSuccess(target);
                    } else {
                        var sleepResist = target.sleepResist;
                        var sleepChance = $gameSystem.randomNum(0, 15);

                        if (sleepChance >= sleepResist) {
                            target.addState(5); // Put Enemy asleep
                            this.makeSuccess(target);
                        } else {
                            result.failedSpell = true;
                        }
                    ...
                    break;
                case 6: // stopspell spell
                    if(target.hasSpellsBlocked()) {
                        this.makeSuccess(target); // If stopspell, stopspell spell will always work
                    } else if (target.isActor()) {
                        var chanceStopspell = $gameSystem.randomNum(1, 100) <= 50;

                        if (chanceStopspell) {
                            target.addState(7); // target has stopspell applied
                            this.makeSuccess(target);    
                        } else {
                            result.failedSpell = true;
                        }
                    } else {
                        var stopspellResist = target.stopspellResist;
                        var chanceStopspell = $gameSystem.randomNum(0, 15);
                        if (chanceStopspell >= stopspellResist) {
                            target.addState(7); // target has stopspell applied
                            this.makeSuccess(target);
                        } else {
                            result.failedSpell = true;
                    ...
                    break;
                case 13:
                    if (target.enemyId() === 25) {
                        target.addState(5); // Put Golem asleep
                        target._timesAsleep = -1; // reset
                        this.makeSuccess(target);
                        
                        result.failedFairyFlute = false;
                    } else {
                        result.failedFairyFlute = true;
                    }
                    // Removing break because we don't want an attack message
                default:
                    result.used = false;
            }
        }
...
Game_Action.prototype.makeDamageValue = function(target, critical) {
    /**
     * 1. Determine if attacker is Hero or Enemy
     * 2. If Enemy 
     */

    var defense;
    var value;
    var rand = $gameSystem.randomNum(0, 255);

    if (this.subject().isActor()) {
        var enemyAgility = target.agi;
        var baseAttack = this.subject().atk
        var attackModifiers = this.subject().weaponsModifier();
        var heroAttack = baseAttack + attackModifiers;

        var result = heroAttack - (enemyAgility / 2);

        if (critical) {
            var attack = heroAttack / 2;
            value = Math.ceil(heroAttack - ((attack * rand) / 256));
        } else if (result < 0 || result === 0 || result === 1) {
            // calculate minimal damage.
            value = $gameSystem.randomNum(0, 1);
        } else {
            
            var attack = result + 1;
            var heroAttackPower = (attack * rand) / 256;

            // regular damage.
            if (heroAttackPower === 0) {
                value = 0;
            } else {
                value = Math.round((heroAttackPower + attack) / 4);
            }
        }
    } else {
        var baseDefense = target.def;
        var armorModifier = target.armorsModifier();
        defense = baseDefense + armorModifier;

        var enemy = this.subject();

        var enemyStrength = enemy.atk;
        var halfedEnemyStrength = (enemyStrength / 2) + 1;
        var heroDefense = defense / 2;
        
        var result = enemyStrength - heroDefense; 

        if (result < 0) {
            /**
             * Lesser damage:
             */
            value = Math.floor((((halfedEnemyStrength * rand) / 256) + 2) / 3);
        } else if (result >= halfedEnemyStrength) {
            /**
             * Regular damage:
             */
            var strengthResult = result; // Store the enemy's strength minus the halved hero's defense at $42.
            result += 1; // Add 1 to $3E

            var enemyAttackPower = (result * rand) / 256; // Multiply the random number by $3E
            if (enemyAttackPower === 0) {
                value = 0;
            } else {
                value = Math.round((enemyAttackPower + strengthResult) / 4);
            }
        } else {
            /**
             * Lesser damage:
...
Game_Action.prototype.heroDefense = function() {
    try {
        var item = this.item();
        var a = this.subject();
        var b = target;
...
Object.defineProperties(Game_BattlerBase.prototype, {
    ...
    // DEFense power
    def: { get: function() { return Math.round(this.param(6) / 2); }, configurable: true },
    // Magic ATtack power
    ...
    // Dodge
    dodge: { get: function() { return this.param(3); }, configurable: true },
    // sleepResist
    sleepResist: { get: function() { return this.param(4); }, configurable: true },
    // stopspellResist
    stopspellResist: { get: function() { return this.param(5); }, configurable: true },
    // hurtResist
    hurtResist: { get: function() { return this.param(7); }, configurable: true },
...
Game_BattlerBase.prototype.stateCount = function(stateId) {
...
Game_Actor.prototype.weaponsModifier = function() {
    var weapons = this.weapons();
    var weaponModifier = 0;

    if (weapons.length !== 0) {
        var weapon = weapons[0];
        weaponModifier = weapon.params[2];
    }

    /**
     * NOTE: There seems to be some debate at whether or not the Fighter's Ring stat
     * is a bug.  Meaning, in the game the Fighter's Ring provide no stat increase 
     * where as the Dragon's Scale add 2+ defense.
     * 
     * It make's sense that it should provide some value other than a random comment from 
     * a dude in Rimuldar.  I have decided to make useful and add 2+ to the attack power.
     */
    var wearingFightersRing = this.states().find(function(state) {  return state.id === 3; });
    if (wearingFightersRing) {
        weaponModifier += 2;
    }
...
Game_Actor.prototype.armorsModifier = function() {
    var armors = this.armors();
    var armorModifier = 0;

    if (armors.length !== 0) {
        armorModifier = armors.reduce(function (prev, curr) {
            var modifier = curr.params[3];
            prev += modifier;
    ...
    var wearingDragonScale = this.states().find(function(state) {  return state.id === 2; });
    if (wearingDragonScale) {
        armorModifier += 2;
    }
...
Game_Actor.prototype.changeEquipById = function(etypeId, itemId) {
    if (etypeId === 3 && itemId === 9) {
        this._magicalArmorSteps = 0;
    }
...
Game_Actor.prototype.changeExp = function(exp, show) {
    this._prevAtk = this.atk;
    this._prevAgi = this.agi;
    this._prevMhp = this.mhp;
    this._prevMmp = this.mmp;
...
Game_Actor.prototype.performDamage = function() {
    ...
    if (this.isSpriteVisible()) {
        ...
        if (this.hp <= 0) {
            $gameScreen.startShake(5, 5, 24);
            $gameScreen.startFlash([255, 0, 0, 225], 6, 4);
        } else {
            $gameScreen.startShake(5, 5, 24);
        }
    }
...
Game_Actor.prototype.showRemovedStates = function() {
    this.result().removedStateObjects().forEach(function(state) {
        if (state.id === 4) { // Repel is removed
            $gameMessage.add("`REPEL has lost its effect.");
        } else {
            if (state.message4) {
...
Game_Enemy.prototype.performEnemyRunsAway = function() {
    SoundManager.playEnemyCollapse();
    this.hide();
    this._didRunAway = true;
};
...
Game_Enemy.prototype.makeActions = function() {
    Game_Battler.prototype.makeActions.call(this);
    var actionList = [];
    var patterns = this._data.pattern;
    ...
    if (this.numActions() > 0) {
        actionList.push({
            ...
            "rating": 5,
            "skillId": 1, // Attack
            "percent": patterns.attack // Percentage of attack
        });

        if (patterns.magic) {
            patterns.magic.forEach(function(magic) {
                var skillId;
                switch (magic.type) {
                    case 'heal':
                        skillId = 12;
                ...
                actionList.push({
                    ...
                    "rating": 5,
                    "skillId": skillId,
                    "percent": magic.percent
                });
            });
        }

        /**
         * Logic to select enemy action.
         * The priority will be to see if the 
         */

        var attack = actionList.find(function(action) { return action.skillId === 1; });
        var healSpell = actionList.find(function(action) { return action.skillId === 12 || action.skillId === 14; });
        var hurtSpell = actionList.find(function(action) { return action.skillId === 13 || action.skillId === 15; });
        var sleepSpell = actionList.find(function(action) { return action.skillId === 4; });
        var stopSpellSpell = actionList.find(function(action) { return action.skillId === 6; });
        var fireBreath = actionList.find(function(action) { return action.skillId === 16 || action.skillId === 17; });

        var hero = $gameParty.members()[0];

        /**
         * If actionList is only 1 then it is attack.
         */
        if (actionList.length === 1) {
            this.action(0).setEnemyAction(attack);
            this.setActionState('waiting');
            return;
        }

        if (healSpell && this.hasLowHp()) {
            if ($gameSystem.randomNum(1, 100) <= healSpell.percent) {
                this.action(0).setEnemyAction(healSpell);
                this.setActionState('waiting');
                return;
            }
        } 

        if (sleepSpell && !hero.isAsleep()) {
            if ($gameSystem.randomNum(1, 100) <= sleepSpell.percent) {
                this.action(0).setEnemyAction(sleepSpell);
                this.setActionState('waiting');
                return;
            }
        }

        if (stopSpellSpell && !hero.hasSpellsBlocked()) {
            if ($gameSystem.randomNum(1, 100) <= stopSpellSpell.percent) {
                this.action(0).setEnemyAction(stopSpellSpell);
                this.setActionState('waiting');
                return;
            }
        }
        ...
        if (fireBreath) {
            if ($gameSystem.randomNum(1, 100) <= fireBreath.percent) {
                this.action(0).setEnemyAction(fireBreath);
                this.setActionState('waiting');
                return;
            }
        }
        ...
        if (hurtSpell) {
            if ($gameSystem.randomNum(1, 100) <= hurtSpell.percent) {
                this.action(0).setEnemyAction(hurtSpell);
                this.setActionState('waiting');
                return;
            }
        }

        // If enemy has no spells or none are used then just attack
        this.action(0).setEnemyAction(attack);
        this.setActionState('waiting');
...
Game_Party.prototype.reviveHero = function() {
    var hero = this.battleMembers()[0];
    if ($gameVariables.value(23)) {
        hero.setHp(1);
    } else {
        hero.setHp(1000);
    }
    ...
    var goldToLose = Math.ceil(gold / 2);
    $gameParty.loseGold(goldToLose); // Lose 1/2 gold on death

    if ($gameVariables.value(23) !== 0) {
        hero.addState(6);
        hero.addState($gameVariables.value(23));
    }
};
...
Game_Troop.prototype.didRunAway = function() {
    return this.members()[0]._didRunAway;
};
...
Game_Player.prototype.performTransfer = function() {
    if (this.isTransferring()) {
        if ($gameSwitches.value(10) && !$gameSwitches.value(11) && !$gameSwitches.value(12)) { // In a Cave and Torch is not lit or Radiant not cast
            $gameScreen.showPicture(1, "dark-default", 0, 0, 0, 100, 100, 255, 0);
            $gameScreen.showPicture(2, "dark-torch", 0, 0, 0, 100, 100, 255, 0);
            $gameScreen.showPicture(3, "dark-radiant-1", 0, 0, 0, 100, 100, 255, 0);
            $gameScreen.showPicture(4, "dark-radiant-2", 0, 0, 0, 100, 100, 255, 0);
        } else if (!$gameSwitches.value(10)) { // Not in a cave
...
Game_Player.prototype.didEncounter = function() {
    // Prevent constant exeuction when stopped
    if (!this.isMoving() && (this.x !== this._prevX || this.y !== this._prevY)) {
        this._prevX = this.x;
        this._prevY = this.y;
        var regionId = this.regionId();

        switch (regionId) {
            case 100: // Zone 100 (e.g. Zone 0)
                switch ($gameMap.tileId(this.x, this.y, 0)) {
                    case 1540: // Grasslands
                        return $gameSystem.randomNum(1, 48) === 1;
                    case 1557: // Bridge
                        return $gameSystem.randomNum(1, 48) === 1;
                    case 1539: // Forest
                        return $gameSystem.randomNum(1, 32) === 1;
                    case 1556: // Hills
                        return $gameSystem.randomNum(1, 32) === 1;
                    case 1548: // Swamp
                        return $gameSystem.randomNum(1, 16) === 1;
                    default:
                        return false;
                }    
            default:
                switch ($gameMap.tileId(this.x, this.y, 0)) {
                    case 1540: // Grasslands
                        return $gameSystem.randomNum(1, 24) === 1;
                    case 1557: // Bridge
                        return $gameSystem.randomNum(1, 48) === 1;
                    case 1539: // Forest
                        return $gameSystem.randomNum(1, 16) === 1;
                    case 1556: // Hills
                        return $gameSystem.randomNum(1, 8) === 1;
                    case 1556: // Desert
                        return $gameSystem.randomNum(1, 8) === 1;
                    case 1548: // Swamp
                        return $gameSystem.randomNum(1, 16) === 1;
...
Game_Player.prototype.executeEncounter = function() {
    if (this.didEncounter()) {
        this.makeEncounterCount();
        var troopId = this.makeEncounterTroopId();
        if ($dataTroops[troopId] && this.notRepelled(troopId)) {
...
Game_Interpreter.prototype.command125 = function() {
    ...
    $gameMessage.add('`Of GOLD thou hast gained ' + value);

    // Hardcoding sound for convenience.
    AudioManager.playSe({ name: '44-Dragon-Quest-1-Open-Treasure', volume: 90, pitch: 100, pan: 0});
    return true;
};
...
Game_Interpreter.prototype.command126 = function() {
    /**
     * Override this so I can use the UI to add items but have the check to see if
     * backpack is full.
     */
    $gameVariables.setValue(17, this._params[0]);
    $gameParty.setItemVars();
