...
DataManager._databaseFiles = [
    ...
    { name: '$dataClasses',      src: 'DW_Classes.json'      },
...
BattleManager.initMembers = function() {
    ...
    this._doesEnemyRunAwayOnEncounter = false;
    ...
    this._dwStatusWindow = null;
    this._dwCommandWindow = null;
    this._usedItem = false;
};

BattleManager.setDWStatusWindow = function(window) {
    this._dwStatusWindow = window;
}

BattleManager.setDWCommandWindow = function(window) {
    this._dwCommandWindow = window;
}
...
BattleManager.onEncounter = function() {
    this._doesEnemyRunAwayOnEncounter = this.isHeroTwiceAsStrongAsEnemy() ? $gameSystem.randomNum(1, 4) === 1 : false;
    this._surprise = this.groupFactorDecision();
};

/**
 * Look, I did not know what to call this.
 * This function is used to determine if Hero is blocked when running away
 * and if the Hero is surpised by enemy.
 */
BattleManager.groupFactorDecision = function() {
    var heroAgility = $gameActors.actor(1).agi;
    var enemyAgility = $gameTroop._enemies[0].agi;
    var enemyId = $gameTroop._enemies[0]._enemyId;
    var groupFactor = this.determineGroupFactorRate(enemyId);
    return (heroAgility * $gameSystem.randomNum(1, 256)) < (enemyAgility * $gameSystem.randomNum(1, 256) * groupFactor);
}

BattleManager.isHeroTwiceAsStrongAsEnemy = function() {
    var heroStrength = $gameActors.actor(1).atk;
    var enemyStrength = $gameTroop._enemies[0].atk;
    return (heroStrength / 2) >= enemyStrength;
}

BattleManager.determineGroupFactorRate = function(enemyId) {
    if (enemyId <= 20) {
        return 0.25;
    } else if(enemyId > 20 && enemyId <= 30) {
        return 0.375;
    } else if(enemyId > 30 && enemyId <= 35) {
        return 0.5;
    } else if(enemyId > 35 && enemyId <= 40) {
        return 1.0;
    }
};
...
BattleManager.update = function() {
    if (!this.isBusy() && !this.updateEvent()) {
        ...
        case 'levelUp':
            ...
            if (!hero._playedLevelUpMusic) {
                AudioManager.stopBgm();
                this._logWindow.push('waitForMessageToPrint');
                this._logWindow.push('playLevelup');
                this._logWindow.push('wait');
                this._logWindow.push('wait');
                this._logWindow.push('wait');
                this._logWindow.push('wait');
                this._logWindow.push('wait');
                this._logWindow.push('replayBgmAndBgs');
                this._logWindow.push('showPauseSignForLevelUp', hero);
                hero._playedLevelUpMusic = true;
                break;
            }
            if (Input.isTriggered('ok') || Input.isTriggered('cancel')) {
                hero._showPauseSignForLevelUp = false;
                this.displayLevelUp();
            }
            break;
        case 'battleEnd':
...
BattleManager.startBattle = function() {
    ...
    this._logWindow.push('displayStatusWindow');
    this._logWindow.push('wait');
};
...
BattleManager.displayStartMessages = function() {
    ...
    $gameTroop.enemyNames().forEach(function(name) {
        enemyName = name; // DW1 you only battled 1 enemy at a time
        that._logWindow.push('addText', TextManager.emerge.format(name));
    });

    // Removing `Preemtive` does not exist in DW1
    if (this._surprise) {
        this._logWindow.push('waitForMessageToPrint');
        that._logWindow.push('addText', TextManager.surprise.format($gameParty.name(), enemyName));
        this._logWindow.push('waitForMessageToPrint');
    }
};
...
BattleManager.startInput = function() {
    ...
    this.clearActor();
    if (this._surprise || this._doesEnemyRunAwayOnEncounter || !$gameParty.canInput()) {
        this.startTurn();
    }
};
...
BattleManager.determineIfStateIsRemoved = function() {
    var hero = $gameParty.members()[0];

    if (hero.isAsleep() && hero._timesAsleep > 0 && hero._timesAsleep < 7) {
        var chanceToWakeUp = $gameSystem.randomNum(1, 100);

        if (chanceToWakeUp <= 50) { // 50/50 chance of waking up
            hero.removeState(5);
        }
    } else if (hero.isAsleep() && hero._timesAsleep === 6) { // Limit sleep to only 6 turns
        hero.removeState(5);
    }
};
...
BattleManager.processTurn = function() {
    ...
    if(subject.isAsleep()) {
        if(subject.isActor()) { // Hero
            // Mirroring limitation in NES
            if (subject._timesAsleep >= 0) {
                subject._timesAsleep++;
            } else {
                subject._timesAsleep = 0;
            }

            this.determineIfStateIsRemoved();

            if (subject.isAsleep()) {
                this._logWindow.push('addText', '|Thou art still asleep.');
            } else {
                this._logWindow.push('addText', '|' + subject.name() + ' awakes.');
                subject._timesAsleep = -1; // Reset
            }
            ...
            // Guarantee atleast 1 attack when asleep
            if (subject._timesAsleep >= 1) {
                var isStillAsleep = $gameSystem.randomNum(1, 3) === 1;  // 1/3 chance of waking up

                if(isStillAsleep) {
                    this._logWindow.push('addText', '|The ' + subject.name() + ' is asleep.');
                } else {
                    subject.removeState(5);
                    this._logWindow.push('addText', '|' + subject.name() + ' hath woken up.');
                    subject._timesAsleep = -1; // reset
                }    
            } else if (subject._timesAsleep === 0) {
                this._logWindow.push('addText', '|The ' + subject.name() + ' is asleep.');
            }
        }
        ...
        if (subject.isActor() && 
           (action.isItem() && action.item().hitType === 0) && // Hero uses item or cast healing spell
           !action.isAttack()) { 
            this._usedItem = true;
        }
...
BattleManager.makeActionOrders = function() {
    ...
    if (!this._surprise && 
        !this._doesEnemyRunAwayOnEncounter &&
        !hero.isAsleep()) {
        battlers = battlers.concat($gameParty.members())
                           .concat($gameTroop.members());
    } else if (this._surprise || hero.isAsleep()) {
        battlers = battlers.concat($gameTroop.members())
                           .concat($gameParty.members());
    } else if (this._doesEnemyRunAwayOnEncounter) {
        battlers = battlers.concat($gameTroop.members());
    }
...
BattleManager.checkBattleEnd = function() {
    if (this._phase) {
        if (this.checkAbort()) {
            ...
            this.processDefeat();
            return true;
        } else if ($gameTroop.isAllDead() && !$gameTroop.didRunAway()) {
            this.processVictory();
            return true;
        } else if ($gameTroop.didRunAway()) {
            this.processEnemyRunningAway()
            return true;
        }
    }
    return false;
...
BattleManager.processEnemyRunningAway = function() {
...
/**
 * Updated processEscape to guarantee a failure.
 * @param {*} shouldFail 
 */
BattleManager.processEscape = function() {
    ...
    if (BattleManager.canEscape()) {
        ...
        // Reversing the logic for determing if Hero can run away
        success = !this.groupFactorDecision();    
    } else {
        success = false;
    }
...
BattleManager.processDefeat = function() {
    $gameSwitches.setValue(65, false); // Turn off dark battle back if you die
    this.endBattle(2);
};
...
BattleManager.endBattle = function(result) {
    ...
    if (result === 0) {
        ...
        this._logWindow.push('wait');
        this._logWindow.push('wait');
        this._logWindow.push('addText', '|Thou art dead.');
        this._logWindow.push('waitForMessageToPrint');
        this._logWindow.push('waitForDeathMusic');
        $gameSwitches.setValue(66, true);
    }
};

BattleManager.updateBattleEnd = function() {
    if (SceneManager._scene._messageWindow.isOpen() && !this._escaped) {
        return;
    }
    ...
    if (this.isBattleTest()) {
        ...
        if (this._canLose) {
            ...
            $gameSwitches.setValue(10, false); //If defeated in a cave turn off torch lighting
            $gamePlayer.reserveTransfer(18, 13, 14, 8, 0); // Throne room infront of the king
            SceneManager.pop();
        }
    } else {
        SceneManager.pop();
...
BattleManager.displayVictoryMessage = function() {
    this._logWindow.push('addText', TextManager.victory.format($gameTroop.deadMembers()[0].name()));
};
...
BattleManager.displayGold = function() {
    ...
    if (gold > 0) {
        ...
        this._logWindow.push('addText', '|increased by ' + gold);
    }
};

BattleManager.displayLevelUp = function() {
    var hero = $gameParty.members()[0];

    if (!hero._playedLevelUpMessage) {
        this._logWindow.push('addText', '|Courage and wit have served thee well. Thou hast been promoted to the next level.');
        this._logWindow.push('waitForMessageToPrint');
        this._logWindow.push('showPauseSignForLevelUp', hero);

        hero._playedLevelUpMessage = true;
    } else if (!hero._playedPowerResponseIncreaseMessage) {
        var power = hero.atk - hero._prevAtk;
        var speed = hero.agi - hero.__prevAgi;

        if (power > 0) {
            this._logWindow.push('addText', '|Thy power increases by ' + power + '.');
        }

        if (speed > 0) {
            this._logWindow.push('addText', 'Thy Response Speed increases by ' + speed + '.');
        }

        this._logWindow.push('waitForMessageToPrint');
        this._logWindow.push('showPauseSignForLevelUp', hero);

        hero._playedPowerResponseIncreaseMessage = true;
    } else if (!hero._playedHipPointMessage) {
        var hp = hero.mhp - hero._prevMhp;
        var mp = hero.mmp - hero._prevMmp;
        this._logWindow.push('addText', '|Thy Maximum Hit Points increase by ' + hp + '.');
        this._logWindow.push('waitForMessageToPrint');

        if (!hero._learnedNewSkill && mp === 0) {
            hero._levelUpped = false;
            hero._playedLevelUpMusic = false;
            hero._playedLevelUpMessage = false;
            hero._playedPowerResponseIncreaseMessage = false;
            hero._playedHipPointMessage = false;
            this._logWindow.push('endBattle');
        } else {
            hero._playedHipPointMessage = true;
            this._logWindow.push('showPauseSignForLevelUp', hero); // Pause arrow for new spell text
        }
    } else if (!hero._magicPointsLearnedNewSkill) {
        var mp = hero.mmp - hero._prevMmp;

        if(mp > 0) {
            this._logWindow.push('addText', '|Thy Maximum Magic Points increase by ' + mp + '.');
        }

        if(hero._learnedNewSkill) {
            hero._learnedNewSkill = false;
            this._logWindow.push('waitForMessageToPrint');
            this._logWindow.push('addText', '|Thou hast learned a new spell.');
            this._logWindow.push('waitForMessageToPrint');
        } else {
            this._logWindow.push('waitForMessageToPrint');
        }
        ...
        hero._playedPowerResponseIncreaseMessage = false;
        hero._playedHipPointMessage = false;
        hero._magicPointsLearnedNewSkill = false;
        this._logWindow.push('endBattle');
    }
};
