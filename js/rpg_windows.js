...
Window_BattleLog.prototype.updateWaitMode = function() {
    ...
    case 'movement':
        ...
        break;
    case 'flash':
        waiting = $gameScreen.isFlashing();
        break;
    case 'shaking':
        waiting = $gameScreen.isShaking();
        break;
    case 'messageToPrint':
        waiting = $gameMessage.hasText() || 
                  SceneManager._scene._messageWindow._textState !== null
        break;
    case 'commandWindow':
        waiting = !SceneManager._scene._dwCommandWindow.isOpenAndActive();
        break;
    }
    if (!waiting) {
        this._waitMode = '';
...
Window_BattleLog.prototype.waitForFlashing = function() {
    this.setWaitMode('flash');
};

Window_BattleLog.prototype.waitForShaking = function() {
    this.setWaitMode('shaking');
};

Window_BattleLog.prototype.waitForMessageToPrint = function() {
    this.setWaitMode('messageToPrint');
};

Window_BattleLog.prototype.waitForDeathMusic = function() {
    $gameSwitches.setValue(14, false);
    $gameSwitches.setValue(15, true);
};

Window_BattleLog.prototype.playCastMagic = function() {
    SoundManager.playUseSkill();
};

Window_BattleLog.prototype.waitForCommandWindow = function() {
    this.setWaitMode('commandWindow');
};
...
Window_BattleLog.prototype.playDragonFire = function() {
    SoundManager.playShop(); // Play Dragon Fire Breath
};

Window_BattleLog.prototype.playLevelup = function() {
    SoundManager.playEvasion(); // Play Improvement music
};

Window_BattleLog.prototype.replayBgmAndBgs = function() {
    BattleManager.replayBgmAndBgs();
};

Window_BattleLog.prototype.showPauseSignForLevelUp = function(subject) {
    subject._showPauseSignForLevelUp = true;
};

Window_BattleLog.prototype.hidePauseSignForLevelUp = function(subject) {
    subject._showPauseSignForLevelUp = false;
};

Window_BattleLog.prototype.endBattle = function() {
...
Window_BattleLog.prototype.performMagicFlash = function() {
    $gameScreen.startFlash([255, 255, 255, 170], 5);
};

Window_BattleLog.prototype.performDeathFlash = function() {
    $gameScreen.startFlash([255, 0, 0, 255], 5);
};
...
Window_BattleLog.prototype.startAction = function(subject, action, targets) {
    if (BattleManager._doesEnemyRunAwayOnEncounter) {
        this.push('clear');
        this.push('addText', '|The ' + subject.name() + ' is running away.');
        this.push('performEnemyRunAway', subject);
        return;
    }
    ...
    this.displayAction(subject, item,  targets[0]);
};
...
Window_BattleLog.prototype.displayCommandWindow = function(subject) {
    SceneManager._scene._dwCommandWindow.showWindow();
    this.displayCommand();
};

Window_BattleLog.prototype.displayStatusWindow = function(subject) {
    SceneManager._scene._dwStatusWindow.show();
};

Window_BattleLog.prototype.displayCommand = function(subject) {
    this.push('addText', '|Command?');
};

Window_BattleLog.prototype.performEnemyRunAway = function(subject) {
    BattleManager._phase = 'turnEnd';
    subject.performEnemyRunsAway();
};
...
Window_BattleLog.prototype.displayAction = function(subject, item, target) {
    ...
    if (DataManager.isSkill(item)) {
        if (item.message1) {
            if (!subject.isActor()) {
                if (item.id === 16 ||
                    ...
                    this.push('waitForMessageToPrint');
                    this.push('playDragonFire');
                    this.push('wait');
                    this.push('wait');
                    this.push('wait');
                } else {
                    ...
                    if (BattleManager._usedItem) {
                        this.push('addText', subject.name() + item.message1.format(item.name));    
                    } else {    
                        if (target.lastBattleSkill() && target.lastBattleSkill().id !== 1) {
                            this.push('addText', subject.name() + item.message1.format(item.name));    
                        } else {
                            this.push('addText', '|' + subject.name() + item.message1.format(item.name));    
                        }
        ...
        if(item.mpCost > 0) {
            this.push('performMagicFlash');
            this.push('playCastMagic');
            this.push('waitForFlashing');
            this.push('performMagicFlash');
            this.push('waitForFlashing');
            this.push('performMagicFlash');
            this.push('waitForFlashing');
        ...
        switch(item.id) {
            case 1:
                this.push('useDragonScale', subject);
                break;
            case 2:
                this.push('useCursedBelt', subject);
                break;
            case 3:
                this.push('useCursedNecklace', subject);
                break;
            case 5:
                this.push('useFightersRing', subject);
                break;
            case 6: // Herbs
                this.push('addText', TextManager.useItem.format(subject.name(), item.name));
                break;
            case 13:
                this.push('playFairyFlute', subject);
                break;
            case 16:
                this.push('playHarp', subject, target);
                break;
        }
    }
...
Window_BattleLog.prototype.useDragonScale = function(subject) {
    // Mirroring commont event 28 for battle
    if (subject.isWearingDragonScale()) {
        this.push('addText', '|Thou art already wearing the scale of the dragon.');
    } else {
        this.push('addText', '|' + subject.name() + ' donned the scale of the dragon.');
        subject.addState(2) // Wearing Dragon Scale
    }
    this.push('waitForMessageToPrint');
    BattleManager._usedItem = false;
};

Window_BattleLog.prototype.useFightersRing = function(subject) {
    // Mirroring commont event 29 for battle
    if (subject.isWearingFightersRing()) {
        this.push('addText', '|' + subject.name() + " adjusted the position of the Fighter's Ring.");
    } else {
        this.push('addText', '|' + subject.name() + " put on the Fighter's Ring.");
        subject.addState(3) // Wearing Dragon Scale
    }
    this.push('waitForMessageToPrint');
    BattleManager._usedItem = false;
};

Window_BattleLog.prototype.useCursedBelt = function(subject) {
    // Mirroring commont event 30 for battle

    if (subject.isWearingCursedBelt()) {
        this.push('addText', "The Cursed Belt is squeezing thy body.\\.");
    } else {
        this.push('addText', '' + subject.name() + " put on the Cursed Belt and was cursed!\\.");
        this.push('waitForMessageToPrint');
        this.push('addText', "|Thy body is being squeezed.");
        subject.addState(6); // Cursed
        subject.addState(8); // Wear Cursed Belt
        $gameVariables.setValue(23, 8);   
    }
    this.push('playCursedMusic');
    this.push('waitForCursedMusic');

    BattleManager._usedItem = false;
};

Window_BattleLog.prototype.useCursedNecklace = function(subject) {
    // Mirroring commont event 31 for battle

    if (subject.isWearingCursedBelt()) {
        this.push('addText', "The Cursed Necklace is squeezing thy body.\\.");
    } else {
        this.push('addText', '' + subject.name() + " put on the Cursed Necklace and was cursed!\\.");
        this.push('waitForMessageToPrint');
        this.push('addText', "|Thy body is being squeezed.");
        subject.addState(6); // Cursed
        subject.addState(9); // Wear Cursed Necklace
        $gameVariables.setValue(23, 9);
    }
    this.push('playCursedMusic');
    this.push('waitForCursedMusic');
...
Window_BattleLog.prototype.playFairyFlute = function(subject) {
    // Mirroring commont event 35 for battle

    this.push('addText', '' + subject.name() + " blew the Fairies' Flute.\\.");
    this.push('waitForMessageToPrint');
    this.push('playFairyFluteMusic');
    this.push('waitForFairyFluteMusic');

    BattleManager._usedItem = false;
};

Window_BattleLog.prototype.playHarp = function(subject, target) {
    // Mirroring commont event 19 for battle

    this.push('addText', '' + subject.name() + " played a sweet melody on the harp.\\.");
    this.push('waitForMessageToPrint');
    this.push('playHarpdMusic');
    this.push('waitForHarpMusic', target);

    BattleManager._usedItem = false;
};

Window_BattleLog.prototype.playHarpdMusic = function() {
    AudioManager.stopBgm();
    AudioManager.playSe({ name: '17-Dragon-Quest-1-Silver-Harp', volume: 90, pitch: 100, pan: 0});
};

Window_BattleLog.prototype.playCursedMusic = function() {
    AudioManager.stopBgm();
    AudioManager.playSe({ name: '23-Dragon-Quest-1-Cursed_', volume: 90, pitch: 100, pan: 0});
};

Window_BattleLog.prototype.playFairyFluteMusic = function() {
    AudioManager.stopBgm();
    AudioManager.playSe({ name: '18-Dragon-Quest-1-Fairy-Flute', volume: 90, pitch: 100, pan: 0});
};

Window_BattleLog.prototype.waitForCursedMusic = function() {
    this.push('wait');
    this.push('wait');
    this.push('wait');
    this.push('wait');
    ...
    this.push('wait');
    this.push('wait');
    this.push('playBattleBgm');
    this.push('wait');
    this.push('wait');
};

Window_BattleLog.prototype.waitForHarpMusic = function(target) {
    this.push('wait');
    this.push('wait');
    this.push('wait');
    this.push('wait');
    ...
    this.push('wait');
    this.push('playBattleBgm');
    this.push('addText', '|' + target.name() + " looks happy.\\.");
    this.push('waitForMessageToPrint');
};

Window_BattleLog.prototype.waitForFairyFluteMusic = function() {
    this.push('wait');
    this.push('wait');
    this.push('wait');
    this.push('wait');
...
Window_BattleLog.prototype.playBattleBgm = function() {
    AudioManager.playBgm({ name: '14-Dragon-Quest-1-A-Monster-Draws-Near', volume: 90, pitch: 100, pan: 0}, 5);
}
...
Window_BattleLog.prototype.displayActionResults = function(subject, target) {
    // This may seem out of place but we want this message to print before the `sleep` message
    if (!target.result().used && 
        !target.isActor() && 
        target.enemyId() === 25 && 
        !target.result().failedFairyFlute){
        this.push('addText', 'Quietly Golem closes his eyes and settles into sleep.');
    } 
    ...
    } else if (!target.isActor() && target.result().failedFairyFlute){
        this.push('addText', '|But nothing happened.');
    }
};

Window_BattleLog.prototype.displayFailure = function(target) {
    if (target.result().isHit() && !target.result().success) {
        if (!target.result().failedSpell && !target.result().spellBlocked) {
            this.push('addText', TextManager.actionFailure.format(target.name()));
        } else if (target.result().spellBlocked) {
            this.push('addText', 'But that spell hath been blocked.');
        } else if(target.result().failedSpell) {
            this.push('addText', 'The spell will not work.');
        }
    }
};
...
Window_BattleLog.prototype.displayCritical = function(target) {
    if (target.result().critical) {
        if (target.isActor()) {
            ...
            SoundManager.playRecovery();  // Excellent move
...
Window_BattleLog.prototype.displayAddedStates = function(target) {
    target.result().addedStateObjects().forEach(function(state) {
        switch (state.id) {
            case 5: // Sleep state
                if (target.isActor()) {
                    this.push('addText', 'Thou art asleep.');
                } else {
                    if (target.enemyId() === 25) {
                        this.push('addText', 'Quietly Golem closes his eyes and settles into sleep.');
                    }

                    this.push('addText', 'Thou hast put the ' + target.name() + ' to sleep.');
                }
                break;
            case 7: // Stopspell state
                if (target.isActor()) {
                    this.push('addText', target.name() + "'s spell is blocked.");
                } else {
                    this.push('addText', 'The ' + target.name() + "'s spell hath been blocked.");
                }
                break;        
        }
